...
from model import ImageRegressionCNN
# from TransCNN import ImageRegressionCNN
...
class Train_Validate():
    ...
    def __init__(self, path, task, device, batch_size_train,
                 ...
                 split_method, num_storms = 10, weight_decay=1.5e-5,
        ...
        if task == 'WindSpeed':
            ...
            # self.model = ImprovedImageRegressionCNN().to(self.device)
            ...
            if self.surprise_storm:
                ...
                print('loading data ......')
                self.ds = Storm_Dataset(self.path,
                                        split_method=self.split_method,num_storms=self.num_storms)
    ...
    def train(self, model, optimizer, criterion, loader):
        if self.task == 'WindSpeed':
            ...
            train_loss = 0
            for X, y in loader:
                X, y = X.to(self.device), y.to(self.device).ToTensor()
                optimizer.zero_grad()
